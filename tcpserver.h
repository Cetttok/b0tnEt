...
{
public:
    TcpServer(const int port); //создает сервер и занимет порт. При аварийном завершении порт и не разрыве подключения порт модет остаться занятым и класс сервера не будет работать.
